...
def cost_of_7king(T: list[list[int]], x :int, y :int=0, suma :int=0) -> int:
    if y == len(T)-1:
        return suma + T[y][x]

    if x==0:
        return min(cost_of_7king(T, x, y + 1, suma + T[y][x]), cost_of_7king(T, x + 1, y + 1, suma + T[y][x]))
    if x == len(T[0])-1:
        return min(cost_of_7king(T, x - 1, y + 1, suma + T[y][x]), cost_of_7king(T, x, y + 1, suma + T[y][x]))

    return min(cost_of_7king(T, x - 1, y + 1, suma + T[y][x]),
               cost_of_7king(T, x, y + 1, suma + T[y][x]),
               cost_of_7king(T, x + 1, y + 1, suma + T[y][x]))
