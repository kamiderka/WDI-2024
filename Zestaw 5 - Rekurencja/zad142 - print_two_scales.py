...
Aby zrobić to po bożemu, powinienem użyć wrappera przekazujące tablice / nonlocala.
Wystarczy podmienić print na append
"""
...
def two_scales(T :list[int], target :int, i :int=0, sum1 :int=0, sum2 :int=0)->bool:
    ...
    if two_scales(T, target, i+1, sum1 + T[i], sum2):
        print("Sum1: ",T[i], end=", ")
        return True

    if two_scales(T, target, i + 1, sum1 , sum2 + T[i]):
        print("Sum2: ",T[i], end=", ")
        return True

    if two_scales(T, target, i + 1, sum1, sum2):
        return True
    return False
